...
- **Checkout:** Send **name**, **email** and optional **state**, **city** (with state), **type**, or none. **Amount is calculated from row count** for that scope (same tiered pricing as list-page / state-page / city-page). Backend stores `data_state` / `data_city` / `data_type` and creates a Stripe Checkout Session. Response returns the session URL for redirect. **Return URLs:** `success_url` = `{GYMSDATA_FRONTEND_URL}/checkout/success?session_id={CHECKOUT_SESSION_ID}`, `cancel_url` = `{GYMSDATA_FRONTEND_URL}/checkout/cancel`. Set `GYMSDATA_FRONTEND_URL` (e.g. `https://gymsdata.gymdues.com`) in env; falls back to `APP_URL` if unset.
